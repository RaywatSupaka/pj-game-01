# Knowledge index

เปิด [rules](rules/README.md) ก่อนใช้ index นี้

| Category | Topic | Status | File |
| --- | --- | --- | --- |
| sources | เอกสารแนวคิดเกมที่ผู้ใช้แนบ | reference-only | [Original design](knowledge/sources/original-game-design.md) |
| product | แนวคิดและเป้าหมายจากเอกสาร | proposed | [Vision](knowledge/product/vision.md) |
| product | Prototype และคำถาม gameplay | proposed | [Prototype](knowledge/product/prototype.md) |
| architecture | Engine และขอบเขตระบบ | accepted engine | [Stack](knowledge/architecture/stack.md) |
| development | เครื่องมือและวิธีเริ่ม dev | observed | [Environment](knowledge/development/environment.md) |
| decisions | Workflow ที่ผู้ใช้กำหนด | accepted | [Workflow decision](knowledge/decisions/0001-workflow.md) |
| product | ไทย/อังกฤษและขอบเขต localization | discovery | [Localization](knowledge/product/localization.md) |
| development | Unity Windows/Android setup | scaffold | [Unity setup](knowledge/development/unity-setup.md) |
| research | Unity platform dependencies | verified docs | [Platforms](knowledge/research/unity-platforms.md) |

## Working documents

- [Foundation plan](plans/0001-foundation.md)
- [Discovery ที่กำลังถาม](discovery/0001-foundation.md)
- [Localization discovery](discovery/0002-localization.md)
- Templates: [discovery](templates/discovery.md), [plan](templates/plan.md), [knowledge](templates/knowledge.md)
